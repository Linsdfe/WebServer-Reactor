...
| 🧵 **One Loop Per Thread** | 每个 IO 线程绑定独立 EventLoop，线程间无锁竞争，**实测QPS超7万**，并发性能优异 |
...
| 📊 **内置性能压测支持** | 集成 wrk 压测流程，提供详细压测指标与结果分析 |
...
- 压测配置：12个压测线程、200个并发连接、持续压测30秒
- 压测目标：服务器本地静态资源 `index.html`
...
| 总处理请求数 | 2218766 |
| 总传输数据量 | 22.83GB |
| **平均QPS (Requests/sec)** | **73759.14** |
| **平均吞吐量 (Transfer/sec)** | **777.21MB** |
| 平均请求延迟 | 3.01ms |
| 最大请求延迟 | 54.84ms |
| 延迟标准差 | 2.90ms |
| 单线程平均每秒请求 | 6.19k |
...
Running 30s test @ http://127.0.0.1:8888/index.html
  ...
  Thread Stats   Avg      Stdev     Max   +/- Stdev
    Latency     3.01ms    2.90ms   54.84ms   85.79%
    Req/Sec     6.19k   629.30    10.81k    69.19%
  2218766 requests in 30.08s, 22.83GB read
Requests/sec:  73759.14
Transfer/sec:    777.21MB
```
...
flowchart TD
    ...
    MethodCheck -->|GET| BuildStatic[构建静态资源响应]
